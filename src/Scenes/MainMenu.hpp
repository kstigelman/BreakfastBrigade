...
#include "Scenes/Screens/TitleScreen.hpp"
#include "Scenes/Screens/CharacterSelectScreen.hpp"
...
public:
    MainMenu (UIRegistry* registry, Game* game) : Scene (game),
        playButton(registry, "Play", sf::Vector2f (308, 440), sf::Vector2f (100, 50)),
        settingsButton(registry, "Settings", sf::Vector2f (308, 490), sf::Vector2f (100, 50)),
        quitButton(registry, "Quit", sf::Vector2f (308, 540), sf::Vector2f (100, 50))
    {
        uiRegistry = registry;
    ...
    void update (float dt) {
        ...
        if (updateScreen) {
            ...
            if (currentPhase == SPLASH) {
                currentPhase = TITLE;
                replaceScreen (new TitleScreen (uiRegistry, getController()));
            }
            if (currentPhase == TITLE) {
                if (identifier == "Play") {
                    currentPhase = SELECT;
                    replaceScreen (new CharacterSelectScreen (uiRegistry, getController ()));
                }
